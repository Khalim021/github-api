...
const form = document.querySelector("#form"),
    ...
    result = document.querySelector(".result-class"),
    searchBtn = document.querySelector("#searchBtn");

const getGitUser = async (username) => {
    const res = await fetch(url_api + username);
    const data = await res.json();
    console.log(data);
    userInfo(data);
}
getGitUser('iamshaunjp');

searchBtn.addEventListener("click", (e) => {
    e.preventDefault();
    const getUser = search.value;
...
const userInfo = (user) => {
    ...
    <div class="card">
        ...
        <div class="user-details"> 
            ...
            <h4 class="username-login">@${user.login}</h4>
